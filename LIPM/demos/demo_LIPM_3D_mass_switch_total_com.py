"""Static results: python LIPM/demos/demo_LIPM_3D_mass_switch_total_com.py
...
class LIPM_3D_Animate:
    def __init__(self, ax):
        ...
        self.COM = Ball(ax, size=12, color='r', label='Whole CoM')
    ...
    def update(self, COM_pos, COM_pos_trajectory, left_foot_pos, right_foot_pos,
        ...
        self.left_leg.update(np.array([body_pos, left_foot_pos]).T)
        self.right_leg.update(np.array([body_pos, right_foot_pos]).T)
        ...
        return [self.origin.scatter, self.COM.scatter, self.body.scatter, self.COM_trajectory.line,
...
def create_model(dt=0.02, t_ss=0.48, t_ds=0.12, height=0.6, velocity=(0.3, 0.),
    ...
    model = LIPM3DMassSwitchTotalCoM(dt=dt,T=t_ss,T_ds=t_ds,
    ...
    model.initializeWholeVelocity(velocity)
    if periodic_start:
...
def main():
    ...
    parser.add_argument('--height', type=float, default=0.6, help='Initial whole-robot CoM height [m]; body height is derived')
    ...
    print(f'Height [m]: initial whole CoM={args.height:g}, fixed body={LIPM_model.body_height:g}, fixed planning={LIPM_model.zc:g}')
    ...
    fig.suptitle(f'Total-CoM initialized mass-switch LIPM ({LIPM_model.body_mass:g}+2x{LIPM_model.foot_mass:g} kg) | SSP {args.ss:.2f}s + DSP {args.ds:.2f}s | '
    ...
    ax.set(xlim=(-1, 4), ylim=(-2, 2), zlim=(-0.01, max(1., LIPM_model.body_height*1.15)),
    ...
    body_2d, = bx.plot([], [], 's', color='black', ms=7)
    ...
    cx.set(xlim=(0, t[-1]), ylim=(min(v.min(), args.vx, args.vy)-0.1,
              ...
              xlabel='time (s)', ylabel='Whole CoM velocity (m/s)')
    COM_vel_x_ani, = cx.plot([], [], color='k', label='Whole CoM velocity x')
    COM_dvel_x_ani, = cx.plot([], [], 'k--', label='desired Whole CoM velocity x')
    COM_vel_y_ani, = cx.plot([], [], color='purple', label='Whole CoM velocity y')
    COM_dvel_y_ani, = cx.plot([], [], color='purple', ls='--', label='desired Whole CoM velocity y')
    dx.set(xlim=(0, t[-1]),
    ...
    def ani_2D_update(i):
        body_2d.set_data([data['body_position'][i,0]], [data['body_position'][i,1]])
        ...
        ani_text_COM_pos.set_text(f'Whole CoM = ({c[i, 0]:.2f}, {c[i, 1]:.2f})')
        bx.set_xlim(c[i, 0]-2, c[i, 0]+3)
        bx.set_ylim(c[i, 1]-0.8, c[i, 1]+0.8)
        return [body_2d] + effective_2d + [COM_traj_ani, zmp_trail, COM_pos_ani, left_foot_pos_ani, right_foot_pos_ani, path_zmp, ani_text_COM_pos]
    ...
    def show_final_result():
        ...
        points = np.vstack((c, z, feet[:, :, :2].reshape(-1, 2), data['body_position'][:, :2], data['effective_com'][:, :, :2].reshape(-1, 2)))
    ...
    print('Mean whole CoM velocity [m/s]:', (c[-1]-c[0])/(t[-1]-t[0]))
