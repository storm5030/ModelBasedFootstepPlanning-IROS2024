"""Run: python LIPM/demo_LIPM_3D_double_support.py
...
class LIPM_3D_Animate:
    def __init__(self, ax):
        ...
        self.COM = Ball(ax, size=16, color='r', label='CoM')
...
def create_model(dt=0.02, t_ss=0.34, t_ds=0.08, height=0.6, velocity=(1.0, 0.),
                 width=0.4, initial_position=None, initial_velocity=None, periodic_start=False):
    """Demo initialization: model construction, foot positions, then CoM state."""
    ...
    LIPM_model = LIPM3DDoubleSupport(dt=dt, T=t_ss, T_ds=t_ds,
                                    s_d=np.linalg.norm(velocity)*(t_ss+t_ds) if periodic_start else 0.6,
                                    w_d=width, support_leg='left_leg')
    if periodic_start:
        left_foot_pos = np.r_[0.5*width*lateral, 0.]
        right_foot_pos = np.r_[-LIPM_model.s_d*forward-0.5*width*lateral, 0.]
    else:
        # Match the explicitly specified initial conditions in the original demo.
        left_foot_pos = [-0.2, 0.3, 0.]
        right_foot_pos = [-0.2, -0.3, 0.]
    COM_pos_0 = [0., 0., height]
    COM_v0 = [1.0, 0.]
    LIPM_model.initializeModel(COM_pos_0, left_foot_pos, right_foot_pos)
    if periodic_start:
        LIPM_model.initializePeriodicState(theta)
    else:
        LIPM_model.vx_0, LIPM_model.vy_0 = COM_v0
        LIPM_model.vx_t, LIPM_model.vy_t = COM_v0
    if initial_position is not None:
        LIPM_model.x_0, LIPM_model.y_0 = np.asarray(initial_position)-LIPM_model.support_foot_pos[:2]
        LIPM_model.x_t, LIPM_model.y_t = LIPM_model.x_0, LIPM_model.y_0
        LIPM_model.COM_pos[:2] = initial_position
    if initial_velocity is not None:
        LIPM_model.vx_0, LIPM_model.vy_0 = initial_velocity
        LIPM_model.vx_t, LIPM_model.vy_t = initial_velocity
        ...
        if LIPM_model.phase == 'SSP':
            ...
            if LIPM_model.phase_count == swing_data_len:
                if double_support_data_len:
                    ...
                    switch_support = True
        elif LIPM_model.phase_count == double_support_data_len:
            switch_support = True
...
def main():
    ...
    parser.add_argument('--ss', type=float, default=0.26, help='Single support duration [s]')
    parser.add_argument('--ds', type=float, default=0.08, help='Double support duration [s]; 0 for baseline')
    ...
    w_d_list = np.array([0.4, 0.8, 0.4, 0.4]) if args.width is None else np.full(4, args.width)
    ...
    fig.suptitle(f'LIPM | SSP {args.ss:.2f}s + DSP {args.ds:.2f}s | '
    ...
    COM_dvel_x_ani, = cx.plot([], [], 'k--', label='desired CoM velocity x')
    COM_vel_y_ani, = cx.plot([], [], color='purple', label='CoM velocity y')
    COM_dvel_y_ani, = cx.plot([], [], color='purple', ls='--', label='desired CoM velocity y')
    dx.set(xlim=(0, t[-1]),
    ...
    def ani_3D_update(i):
        COM_pos = [c[i, 0], c[i, 1], LIPM_model.zc]
    ...
    def ani_2D_update(i):
        ...
        ani_text_COM_pos.set_text(f'COM = ({c[i, 0]:.2f}, {c[i, 1]:.2f})')
        bx.set_xlim(c[i, 0]-2, c[i, 0]+3)
        bx.set_ylim(c[i, 1]-0.8, c[i, 1]+0.8)
        return [COM_traj_ani, zmp_trail, COM_pos_ani, left_foot_pos_ani, right_foot_pos_ani, path_zmp, ani_text_COM_pos]
    ...
    if args.headless:
        # Exercise rendering in both phases, including the final state.
        for i in (0, min(len(t)-1, round(args.ss/args.dt)), len(t)-1):
            _update_func(i)
            fig.canvas.draw()
            assert all(len(line.get_xdata()) == i+1 for line, _ in histories)
            assert len(COM_traj_ani.get_xdata()) == i+1
        plt.close(fig)
    else:
        _update_func(0)
    ...
    print('Mean velocity [m/s]:', (c[-1]-c[0])/(t[-1]-t[0]))
